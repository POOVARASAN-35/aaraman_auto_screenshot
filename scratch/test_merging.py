...
def detect_presentation_segment_merged(img_path, name, max_gap=100):
    ...
    # Filter segments by minimum width
    segments = [seg for seg in segments if (seg[1] - seg[0]) >= 10]
    
    # Merge adjacent segments separated by a small gap
    ...
    merged_segments = merge_adjacent_segments(segments, max_gap)
    
    print(f"\n--- Merged Detection on {name} ---")
    print(f"  Original segments: {[(s, e, e - s) for s, e in segments]}")
    print(f"  Merged segments:   {[(s, e, e - s) for s, e in merged_segments]}")
    
    # Filter out the participant panel
    valid_segments = []
    ...
    # Crop vertically inside this segment
    ...
    out_path = f"Output/merged_{name}.png"
...
if __name__ == "__main__":
    for pct in [5, 25, 50, 75]:
        ...
        if os.path.exists(path):
            detect_presentation_segment_merged(path, f"sample_{pct}")
